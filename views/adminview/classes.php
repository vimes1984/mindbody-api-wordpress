	...
	*/
	...
	if($sourcename != "" && $password != "" && $siteID != ""  && $fromdate != "" && $todate != "" && $_GET ["settings-updated"] == 'true'){
		...
		foreach ($resultsflipped as $key => $value) {
				...
				$StartDate = new DateTime($value->StartDateTime);
				$stripStartDate = $StartDate->format('l jS \of F Y');
				$EndDate = new DateTime($value->EndDateTime);
				$stripEndDate = $EndDate->format('l jS \of F Y');
				$weekday = $StartDate->format('l\s');
				...
				*/ 
				...
				//first check does the class allready exist...
			  	if($checktypeofreturn == 'string') {
						...
						update_post_meta( $id_ofpost_title, 'wpcf-starttime', date( "H:i",strtotime( $value->StartDateTime ) ) );
						update_post_meta( $id_ofpost_title, 'wpcf-endtime', date("H:i",strtotime($value->EndDateTime)));
						...
						update_post_meta( $id_ofpost_title, 'wpcf-week-day', $weekday);

						// Update the post post_excerpt
						wp_update_post( array('ID' =>  $id_ofpost_title, 'post_excerpt' => $value->Description ) );
						wp_set_object_terms( $id_ofpost_title, 'dropin-class', 'product_cat', true );
						...
						update_post_meta( $prodid, 'wpcf-starttime', date( "H:i",strtotime( $value->StartDateTime ) ) );
						update_post_meta( $prodid, 'wpcf-endtime', date("H:i",strtotime($value->EndDateTime)));
						...
						update_post_meta( $prodid, 'wpcf-week-day', $weekday);
						wp_set_object_terms( $prodid, 'dropin-class', 'product_cat', true );
			...
			<div id="postbox-container-1" class="postbox-container">				
				<div class="meta-box-sortables">
					<div class="postbox">
						...
						<div class="inside">
							<p>You can display classes on page using a shortcode:</p>
			...
			<div id="post-body-content">
				<div class="meta-box-sortables ui-sortable">
					<div class="postbox">
						...
						<div class="inside">
							<p>Pick Classes  date range to import:</p>
							...
							<form method="post" action="options.php">
								<?php
									...
									submit_button( 'IMPORT', 'primary', 'submit', true, '' );
								?>
							</form>
						</div>
					</div>
